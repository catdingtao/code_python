...
th = tr[0].find_all('th')
print(th)
for th_num in range(1,len(th)):
    th_value =th[th_num-1].get_text()
    print(th_value)
for tr_num in range(1,len(tr)):
    tds = tr[tr_num].find_all('td')
    for tds_num  in range(1,len(tds)):
        tds_value=tds[tds_num-1].get_text()
        print("tr[%s]td[%s]:   %s"%(tr_num-1,tds_num-1,tds_value))
